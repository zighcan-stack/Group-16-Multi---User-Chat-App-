import tkinter as tk                       # import the tkinter GUI library and alias it as 'tk'
from tkinter import ttk, simpledialog, messagebox  # import themed widgets, simple dialog, and messagebox utilities from tkinter
import time                                # import time module for timestamps
import socket                              # import socket module for network communication
import threading                           # import threading to run background tasks without freezing the UI

# ====== CONFIG ======
SERVER_HOST = "127.0.0.1"   # default localhost (will be overridden if user provides ngrok address)
SERVER_PORT = 12345         # default port (will be overridden if user provides ngrok port)

# ====== SOCKET ======
client_socket = None        # will hold the socket object once connected to server
username = None             # will hold the username string provided by the user
running = True              # flag used to control the receive loop and app shutdown

# ====== UI ======
app = tk.Tk()               # create the main application window
app.title("MultiChat App")  # set the window title
app.geometry("800x700")     # set the initial window size (width x height)
app.configure(bg="#121212") # set the background color of the window
...
def connect_to_server():                                                   
    global client_socket, username, SERVER_HOST, SERVER_PORT                
    ...
    # Ask for ngrok server details
    host_input = simpledialog.askstring("Server Address", "Enter server host (e.g. 4.tcp.eu.ngrok.io):", parent=app)
    port_input = simpledialog.askinteger("Server Port", "Enter server port (e.g. 13865):", parent=app)

    if host_input and port_input:
